...
df = pd.read_csv('data/LowerAttendance.csv')
...
def end_value(data, level):
    ...
    data = data.astype({' Home Win %': 'float64', ' Visitor Win %': 'float64', ' Home Market Index': 'float64', ' Home Stars Index': 'int64', ' Visitor Stars Index': 'int64', ' Home AFP': 'int64', ' Visitor AFP': 'int64', ' Ticket Price': 'float64', ' Lower Section Attendance': 'int64'})
    ...
    data['pct'] = data[' Lower Section Attendance'] / n
...
model_df = end_value(data=df, level='lower')
...
joblib.dump(model, 'model_objects/lower_model.pkl')
