...
df = pd.read_csv('data/UpperAttendance.csv')
...
def end_value(data, level):
    ...
    data = data.astype({' Home Win %': 'float64', ' Visitor Win %': 'float64', ' Home Market Index': 'float64', ' Home Stars Index': 'int64', ' Visitor Stars Index': 'int64', ' Home AFP': 'int64', ' Visitor AFP': 'int64', ' Ticket Price': 'float64', ' Upper Section Attendance': 'int64'})
    ...
    data['pct'] = data[' Upper Section Attendance'] / n
...
model_df = end_value(data=df, level='upper')
...
joblib.dump(model, 'model_objects/upper_model.pkl')
